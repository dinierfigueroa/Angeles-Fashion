...
import { collection, query, onSnapshot, orderBy, where, Timestamp, doc, getDocs, limit } from 'firebase/firestore';
...
const fetchDeposits = () => {
    ...
    if (!userPermissions.value.canViewAllDeposits) {
        if (userAccessibleStores.value.length > 0) {
            if (statusValue === 'reservado' || statusValue === 'liquidado') {
...
<template>
  <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div>
          ...
          <select v-model="filters.status" class="mt-1 block w-full pl-3 pr-10 py-2 border-gray-300 rounded-md">
            ...
            <option value="liquidado">Finalizados</option>
            <option v-if="userPermissions.canViewAllDeposits" value="todos">Todos</option>
          </select>
        </div>
        <div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-else-if="filteredDeposits.length === 0"><td colspan="7" class="text-center py-4 text-gray-500">No se encontraron depósitos con los filtros actuales.</td></tr>
            <tr v-for="d in filteredDeposits" :key="d.id">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                <button v-if="d.status === 'disponible'" @click="openReserveModal(d)" class="text-indigo-600 hover:text-indigo-900">Reservar</button>
                <button v-if="d.status === 'disponible' || d.status === 'reservado'" @click="openLiquidateModal(d, d.status === 'disponible')" class="text-green-600 hover:text-green-900">Liquidar</button>
                <button v-if="d.status && finalizedStatuses.includes(d.status) && userPermissions.canRevertDeposits" @click="openRevertModal(d)" class="text-red-600 hover:text-red-900">Revertir</button>
    ...
    <ReserveDepositModal v-model="isReserveModalOpen" :deposit="selectedDeposit" :store-id-filter="!userPermissions.canViewAllDeposits ? userAccessibleStores.map(ref => ref.id) : null" @deposit-updated="handleDepositUpdated" />
    <LiquidateDepositModal v-model="isLiquidateModalOpen" :deposit="selectedDeposit" :action="modalAction" :direct-liquidation="isDirectLiquidation" :store-id-filter="!userPermissions.canViewAllDeposits ? userAccessibleStores.map(ref => ref.id) : null" @deposit-updated="handleDepositUpdated" />
